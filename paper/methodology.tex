...
\fixme{PAM-reference}. First we scan the entire IPv4 address space using
\texttt{zmap}\fixme{cite} using a DNS query on port 53 for a control domain that
...
address for uncensored domains we control using \texttt{zdns}\fixme{cite}. By
...
Censored Planet's Satellite project\fixme{cite} has cultivated a list of domains
...
\texttt{zgrab2}\fixme{cite} and locally verify the returned TLS certificates.
