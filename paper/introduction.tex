...
%% hook
\para{The IP transition introduces inconsistencies in security policies.}
...
\begin{itemize}
  ...
  \item {\bf Methodological improvements (\Cref{sec:methods}.} We begin by
  ...
  \item {\bf Prevalence of DNS censorship (\Cref{sec:prevalence}).} We conduct
    a large-scale measurement of DNS censorship in six countries with IPv6
    connectivity. We find \rnnote{fill in nice result summary here.}

  \item {\bf Differences due to DNS query type (\Cref{sec:query}).} We then
    ...
    record). We find that \rnnote{summary}.

  \item {\bf Differences due to network infrastructure (\Cref{sec:dual}).}
