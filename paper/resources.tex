...
\Cref{sec:methodology} for data collection methodology). We then apply the censorship determination methods described in
\Cref{sec:methodology:censorship} to measure the prevalence of censorship on our {\tt
...
where $dom$ is the total number of tested domains (717). We only label the
...
a specific content-specific policy gap that exists in the censorship mechanism
...
in a range of corporate networks (\Cref{sec:prevalence}) which are fairly
consistent in their blocking of domains belonging to McAfee's `Potentially
unwanted programs', `Entertainment', and `Pornography' categories. 
%
On the other hand, in Thailand the low divergence appears despite the large
...
a simply incomplete implementation of any existing mechanisms for {\tt A} query
censorship. 
%
