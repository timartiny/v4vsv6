...
($\nabla_{\text{query}}$) metric \cite{KLdiversity}. In simple terms, the
...
\begin{table*}[t]
  ...
  \scalebox{\tabularscale} {
    ...
    \toprule
      ...
      \midrule
      Thailand (TH)       & 186 & 152 (81.7\%)  &  AS9835 Government IT Services (40)  & 4.50 & 4.10 & 0.14 & Cable/DSL (110) \\
  ...
  ASes, and `$\nabla_{AS}$' represents the Kullback-Leibler divergence of the
  distribution of inconsistent resolvers from the distribution of all resolvers
  in the country's ASes (\cf \Cref{sec:resources:resolvers}).
  %
