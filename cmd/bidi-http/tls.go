...
func (p *tlsProber) buildPaylaod(name string, r *rand.Rand) ([]byte, error) {
	...
	// TODO dynamic
	// client Rand (32 generated bytes)
	buf := make([]byte, 32)
	n, err := r.Read(buf)
	if err != nil || n != 32 {
	...
	// TODO dynamic
	// session ID (33 bytes 32 generated)
	n, err = r.Read(buf)
	if err != nil || n != 32 {
		return nil, fmt.Errorf("failed rand read: %s", err)
	}
...
func (p *tlsProber) sendProbe(ip net.IP, name string, lAddr string, timeout time.Duration, verbose bool) (*Result, error) {
	...
	rawBytes, err := p.buildPaylaod(name, r)
	if err != nil {
