...
cat $OUTPUTFOLDER/v4_cartesian_AAAA_lookups_$DATESTR.json | jq -r '.name as $name | .data.answers[]? | select(.type=="A") | "\(.answer), \($name)"' >> $OUTPUTFOLDER/tmp
...
cat $OUTPUTFOLDER/v4_cartesian_A_lookups_$DATESTR.json | jq -r '.name as $name | .data.answers[]? | select(.type=="AAAA") | "\(.answer), \($name)"' >> $OUTPUTFOLDER/tmp
...
cat $OUTPUTFOLDER/v6_cartesian_AAAA_lookups_$DATESTR.json | jq -r '.name as $name | .data.answers[]? | select(.type=="A") | "\(.answer), \($name)"' >> $OUTPUTFOLDER/tmp
...
cat $OUTPUTFOLDER/v6_cartesian_A_lookups_$DATESTR.json | jq -r '.name as $name | .data.answers[]? | select(.type=="AAAA") | "\(.answer), \($name)"' >> $OUTPUTFOLDER/tmp
